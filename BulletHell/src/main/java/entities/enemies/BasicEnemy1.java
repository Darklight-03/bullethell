...
public class BasicEnemy1 extends EntityBase implements PolygonHitBox {
	...
	public boolean update() {
		...
		if (GameManager.getGame().getCount() % (int) ((Config.UPS * Config.GAME_SPEED) / RoF) == 0) {
			...
			while (bps > ct) {
				...
				GameManager.getGame().projectiles.add(new BasicShot(Config.UGLY_PLACEHOLDER_PROJECTILE, Math
	...
	public void updateXPoints() {
		...
		for (int i = 1; i < 16; i++) {
			...
			// System.out.print((int) (x + radius * Math.cos(((i - 1) * 24 *
			// Math.PI) / 180))+", ");
	...
	public void updateYPoints() {
		...
		for (int i = 1; i < 16; i++) {
			...
			// System.out.print((int)(x + radius * Math.sin(((i - 1) * 24 *
			// Math.PI) / 180))+", ");
